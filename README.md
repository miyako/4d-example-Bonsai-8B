## [prism-ml/Bonsai-8B-gguf](https://huggingface.co/prism-ml/Bonsai-8B-gguf)

> [!NOTE]
> The [`prism-ml`](https://github.com/miyako/llama-cpp/tree/prism-ml) branch of `miyako/llama-cpp` is based on [`PrismML-Eng/llama.cpp`](https://github.com/PrismML-Eng/llama.cpp)

## Example

<img width="630" height="428" alt="" src="https://github.com/user-attachments/assets/37a71be7-8e36-4689-822f-864bb7d9356b" />

## Memory Consumption

<img width="740" height="384" alt="" src="https://github.com/user-attachments/assets/e73ed1c3-9eeb-46b6-ae9a-27254c5dfa7c" />
